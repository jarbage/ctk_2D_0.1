...
function setOutOfBounds(polygonNodeArray){
    ...
    for(let i in polygonNodeArray){
        ...
        let checkColumn = parseFloat($(polygonNode).attr('column'));
        if (checkColumn > columnRef){
            columnRef = checkColumn;
        }
    ...
    for(let i in polygonNodeArray){
        let polygonNode = polygonNodeArray[i];
        let checkRow = parseFloat($(polygonNode).attr('row'));
        if (checkRow > rowRef){
            rowRef = checkRow;
        }
    }
    ...
    for(let i in polygonNodeArray){
        let polygonNode = polygonNodeArray[i];
        if($(polygonNode).attr('row')== 0 || $(polygonNode).attr('column') == 0){
            $(polygonNode).attr('OOB', true);
            $(polygonNode).remove();
        }
        if($(polygonNode).attr('row') == rowMax || $(polygonNode).attr('column') == columnMax ){
                    $(polygonNode).attr('OOB', true);
                    $(polygonNode).remove();
        }
    }
...
function setForest(polygonNodeArray){
    for(let i in polygonNodeArray){
        ...
        if( ( polygonRow == 0.5)  ||   ($(polygonNode).attr('row') == 1)  
         || ( ($(polygonNode).attr('row')    == 0.5)   ||   ($(polygonNode).attr('row') == (rowMax - 0.5)) )
         || ( ($(polygonNode).attr('column') == 0.5)) ||($(polygonNode).attr('column') == (columnMax - 0.5)) 
         || ( ($(polygonNode).attr('row') == 1)) ||($(polygonNode).attr('row') == (rowMax - 1))){
